        ...
        for account in self.accounts:
            ...
            for region in regions:
                ...
                for sg in sgs:
                    ...
                    # Retrieve all instances using this sg
                    sg_filter = {"instance.group-id" : sg.id}
                    instances = self.wrap_aws_rate_limited_call(rec2.get_only_instances, filters=sg_filter)
                    ...
                    for i in instances:
                        ...
                        (inst_name,) = inst_tags
                        if inst_name:
                            assign_inst.append("%s (%s)" % (i.id, inst_name))
                        else:
                            assign_inst.append("%s" % i.id)
                    if assign_inst:
